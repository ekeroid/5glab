...
class EdgeVisionGUI:
    def __init__(self):
        ...
        self._zone_picker_active = False
        self._zone_picker_zones: list[dict] = []
        self._zone_picker_selected = 0

        # Edge setup progress (refreshed on every poll while CONNECTING)
        self._setup_progress: dict | None = None
    ...
    def _toggle_mode(self):
        ...
        if self._mode == "local":
            # Always offer the zone picker on LOCAL → EDGE, even if we already
            # have an instance from earlier in the session. If the user keeps
            # the same zone, _setup_edge will reuse the existing endpoint;
            # if they pick a different one we reinitialise cleanly.
            self._start_zone_picker()
        else:
            self._mode = "local"
            with self._lock:
                self._latest_timing = {}
    ...
    def _start_zone_picker(self):
        self._log_event("..", "Discovering edge zones...")
        threading.Thread(target=self._discover_zones_bg, daemon=True).start()

    def _discover_zones_bg(self):
        try:
            zones = camara.discover_zones()
            self._zone_picker_zones = zones
            self._zone_picker_selected = 0
            self._zone_picker_active = True
            self._log_event("OK", f"Found {len(zones)} zone(s) — pick with UP/DOWN, ENTER to confirm")
        except Exception as e:
            self._log_event("!!", f"Zone discovery failed: {e}")

    def _confirm_zone_picker(self):
        if not self._zone_picker_zones:
            return
        zone = self._zone_picker_zones[self._zone_picker_selected]
        self._zone_picker_active = False
        self._mode = "edge"
        self._edge_setup_in_progress = True
        self._event_log.clear()
        self._log_event(">>", f"EDGE OFFLOAD INITIATED — zone: {zone.get('edgeCloudZoneName')}")
        threading.Thread(target=self._setup_edge, args=(zone,), daemon=True).start()

    def _cancel_zone_picker(self):
        self._zone_picker_active = False
        self._zone_picker_zones = []
        self._log_event("<<", "Zone selection cancelled")

    def _setup_edge(self, zone: dict = None):
        try:
            # Step 1: Use pre-selected zone or discover
            if zone is None:
                self._log_event("..", f"Discovering edge zones at {config.CAMARA_API_URL}")
                t0 = time.perf_counter()
                zone = camara.discover_zone()
                ms = (time.perf_counter() - t0) * 1000
                self._log_event("OK", f"Zone found [{ms:.0f}ms]")

            zone_id = zone["edgeCloudZoneId"]
            zone_name = zone.get("edgeCloudZoneName", zone_id)
            gpu = zone.get("capabilities", {}).get("gpuModel", "N/A")
            self._log_event("OK", f"Zone: {zone_name} (GPU: {gpu})")

            # Fast path: same zone as the existing instance, endpoint still alive.
            # Skip register/instantiate/poll to avoid evicting a warm pod (the
            # NEF-shim's replace-on-409 would otherwise tear it down).
            if (self._edge_instance_id
                    and self._edge_endpoint
                    and zone_id == getattr(self, "_edge_zone_id", None)):
                self._log_event("**", f"EDGE OFFLOAD ACTIVE — reusing warm instance "
                                       f"{self._edge_instance_id[:8]}... on {zone_name}")
                self._edge_setup_in_progress = False
                return

            # Different zone (or no live instance): tear down anything we had
            # so the new instance's slug/endpoint replaces it cleanly.
            if self._edge_instance_id:
                self._log_event("..", "Switching zones — releasing previous instance")
                try:
                    camara.terminate_app(self._edge_instance_id)
                except Exception as e:
                    self._log_event("!!", f"  teardown warning: {e}")
                self._edge_instance_id = None
                self._edge_endpoint = None
                self._edge_grpc_endpoint = None
                self._edge_http_endpoint = None
            ...
            self._edge_zone_id = zone_id
            ms = (time.perf_counter() - t0) * 1000
            self._log_event("OK", f"Instance created: {instance_id[:8]}... [{ms:.0f}ms]")

            # Step 4: Wait for ready (V100 cold build of seg engine can take ~15 min)
            timeout_s = 1500
            poll_every_s = 2
            self._log_event("..", f"Waiting for GPU instance (timeout {timeout_s}s)...")
            t0 = time.perf_counter()
            last_phase = ""
            last_message = ""
            phase_start = t0
            self._setup_progress = {"phase": "scheduling", "started_at": t0,
                                     "phase_started_at": t0, "message": ""}
            poll_count = max(1, timeout_s // poll_every_s)
            for poll_i in range(poll_count):
                if self._mode != "edge":
                ...
                # Sync UI progress state (uses local clock; phase_started_at is
                # local-perf time of the *first* poll that saw this phase)
                ...
                raise RuntimeError(f"Timeout waiting for instance ({timeout_s}s)")
            ...
            t0 = time.perf_counter()
            endpoints = camara.get_endpoints(instance_id)
            grpc_ep = endpoints.get("grpc", {}).get("url") or None
            http_ep = endpoints.get("http", {}).get("url") or None
    ...
    # Phase progress bar shown while CONNECTING — segments and per-phase
    # expected durations (seconds) used to drive the within-phase fill.
    _PHASE_BAR = [
        ("scheduling", "schedule", 5),
        ("pulling",    "pull",     360),
        ("starting",   "start",    20),
        ("running",    "warmup",   60),
        ("ready",      "ready",    1),
    ...
    def _draw_top_bar(self, canvas: np.ndarray):
        ...
        if self._edge_setup_in_progress and self._setup_progress:
            self._draw_setup_progress(canvas)
            return
        ...
        lat_colour = (0, 255, 100) if self._mode == "edge" else (0, 180, 255)
    ...
    def _draw_setup_progress(self, canvas: np.ndarray):
        """Draw the multi-segment phase progress bar in the top bar."""
        prog = self._setup_progress or {}
        current_phase = prog.get("phase", "")
        phase_started_at = prog.get("phase_started_at") or time.perf_counter()
        started_at = prog.get("started_at") or time.perf_counter()
        ...
        n = len(self._PHASE_BAR)
        seg_w = bar_w // n
        cur_idx = next((i for i, (p, _, _) in enumerate(self._PHASE_BAR)
                        if p == current_phase), -1)

        for i, (phase_name, label, expected) in enumerate(self._PHASE_BAR):
            sx = bar_x + i * seg_w
            sw = seg_w if i < n - 1 else (bar_w - i * seg_w)

            if cur_idx < 0:
                # Unknown phase
                fill_colour = (60, 60, 60)
                fill = 0
            elif i < cur_idx:
                # Past phase — full green
                fill_colour = (40, 140, 40)
                fill = sw
            elif i == cur_idx:
                # Current phase — animated fill, capped at 95% so it's clear
                # the phase isn't done yet
                ratio = min(0.95, phase_elapsed / max(1.0, expected))
                fill_colour = (0, 140, 200) if phase_name != "ready" else (40, 200, 40)
                fill = int(sw * ratio)
            else:
                # Future phase
                fill_colour = (50, 50, 55)
                fill = 0

            if fill > 0:
                cv2.rectangle(canvas, (sx + 1, bar_y + 1),
                              (sx + fill - 1, bar_y + bar_h - 1),
                              fill_colour, -1)
            # Segment divider
            if i > 0:
                cv2.line(canvas, (sx, bar_y), (sx, bar_y + bar_h),
                         (15, 15, 20), 1)
            # Label
            text_colour = (255, 255, 255) if i == cur_idx else (170, 170, 170)
            (tw, _), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
            cv2.putText(canvas, label,
                        (sx + (sw - tw) // 2, bar_y + 16),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, text_colour, 1)

        # Right-side: total elapsed + current message (truncated)
        timer = f"{total_elapsed:6.1f}s"
        cv2.putText(canvas, timer, (WINDOW_W - 195, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (220, 220, 220), 1)
        # Status message under the bar
        msg_short = (message[:90] + "…") if len(message) > 90 else message
        cv2.putText(canvas, msg_short, (bar_x, bar_y + bar_h + 12),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.36, (160, 200, 230), 1)
    ...
    def _draw_zone_picker(self, canvas: np.ndarray):
        zones = self._zone_picker_zones
        if not zones:
            return
        ...
        # Box
        cv2.rectangle(canvas, (x0, y0), (x0 + box_w, y0 + box_h), (50, 50, 50), -1)
        cv2.rectangle(canvas, (x0, y0), (x0 + box_w, y0 + box_h), (100, 100, 100), 2)

        # Title
        cv2.putText(canvas, "SELECT EDGE ZONE", (x0 + 20, y0 + 28),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

        # Zone rows
        for i, z in enumerate(zones):
            ry = y0 + 40 + i * row_h
            selected = i == self._zone_picker_selected

            if selected:
                cv2.rectangle(canvas, (x0 + 10, ry), (x0 + box_w - 10, ry + row_h - 4), (0, 120, 200), -1)
            else:
                cv2.rectangle(canvas, (x0 + 10, ry), (x0 + box_w - 10, ry + row_h - 4), (40, 40, 40), -1)

            name = z.get("edgeCloudZoneName", z["edgeCloudZoneId"])
            caps = z.get("capabilities", {})
            gpu = caps.get("gpuModel", "CPU only") if caps.get("gpuAvailable") else "CPU only"
            provider = z.get("edgeCloudProvider", "")

            cv2.putText(canvas, f"[{i+1}] {name}", (x0 + 20, ry + 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(canvas, f"    {provider} | {gpu}", (x0 + 20, ry + 38),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (180, 180, 180), 1)

        # Footer
        fy = y0 + box_h - 18
        cv2.putText(canvas, "UP/DOWN or 1/2 to select | ENTER to confirm | ESC to cancel", (x0 + 20, fy),
    ...
    def _render(self) -> np.ndarray:
        ...
        if self._zone_picker_active:
            self._draw_zone_picker(canvas)
    ...
    def run(self):
        ...
        while self._running:
            ...
            if self._zone_picker_active:
                n = len(self._zone_picker_zones)
                if key == 27:
                    self._cancel_zone_picker()
                elif key == 13 or key == 10:
                    self._confirm_zone_picker()
                elif key == 0 or key == 82:  # UP arrow
                    self._zone_picker_selected = (self._zone_picker_selected - 1) % n
                elif key == 1 or key == 84:  # DOWN arrow
                    self._zone_picker_selected = (self._zone_picker_selected + 1) % n
                elif ord('1') <= key <= ord('0') + n:
                    self._zone_picker_selected = key - ord('1')
                    self._confirm_zone_picker()
            else:
...
def main():
    ...
    print()
    print("Controls:")
    print("  [E] or click button — Toggle LOCAL/EDGE mode (shows zone picker)")
